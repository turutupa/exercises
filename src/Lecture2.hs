...
lazyProduct list = go list 1
  where
    ...
    go ls prod
      | null ls = prod
      | head ls == 0 = 0
      | otherwise = go (tail ls) (prod * head ls)
...
removeAt :: Int -> [Int] -> Result
removeAt n xs
  | length xs >= n = Justme (xs !! n) (take (n -1) xs ++ drop n xs)
  | otherwise = Nothingme xs

showResult :: Result -> String
showResult (Justme int list) = "Just " ++ show int ++ ", " ++ show list
showResult (Nothingme list) = "Nothing, " ++ show list
...
-- data UserReplica = MkUserReplica
--   { userName :: String,
--     userAge :: Int,
--     userIsTired :: Bool
--   }
